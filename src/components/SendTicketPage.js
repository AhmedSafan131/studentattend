...
const SendTicketPage = ({ doctorName, doctorEmail }) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (!validate()) return;
    const ticket = {
      id:        `TKT-${String(localId++).padStart(3, '0')}`,
      subject:   form.subject,
      category:  CATEGORIES_EN[form.category],
      priority:  form.priority,
      body:      form.body,
      from:      doctorName  || 'Doctor',
      email:     doctorEmail || '',
      role:      'doctor',
      status:    'open',
  ...
  const statusLabel = {
    open:       `🟡 ${t('ticketOpen')}`,
    inProgress: `🔵 ${t('ticketInProgress')}`,
    resolved:   `✅ ${t('ticketResolved')}`,
    closed:     `⚫ ${t('ticketClosed')}`,
  };

  const statusColor = {
    open: '#f59e0b', inProgress: '#60a5fa', resolved: '#34d399', closed: '#6b7280',
  };
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: 24, alignItems: 'start' }}>
        ...
        <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          {submitted && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px', borderRadius: 10, background: 'rgba(52,211,153,0.1)', border: '1px solid rgba(52,211,153,0.3)' }}>
              ...
              <div>
                <div style={{ fontSize: 13, fontWeight: 700, color: '#34d399' }}>{t('ticketSuccess')}</div>
                <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 2 }}>{t('ticketSuccessSub')}</div>
              </div>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
            ...
            <div>
              ...
              <input value={form.subject} onChange={e => set('subject', e.target.value)}
                ...
                style={{ ...inp, borderColor: errors.subject ? '#ef4444' : '' }}
              />
            ...
            <div>
              ...
              <textarea rows={6} value={form.body} onChange={e => set('body', e.target.value)}
                ...
                style={{ ...inp, resize: 'vertical', borderColor: errors.body ? '#ef4444' : '' }}
              />
            ...
            <button type="submit" style={{
              ...
              boxShadow: '0 4px 14px rgba(26,107,69,0.3)',
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
            <div style={{ padding: '14px 18px', borderBottom: '1px solid var(--border)', fontSize: 12, fontWeight: 700, color: 'var(--text-muted)' }}>
              {t('myTickets')} ({myTickets.length})
            </div>
            {myTickets.length === 0 ? (
              <div style={{ padding: '30px 20px', textAlign: 'center', color: 'var(--text-muted)' }}>
                <div style={{ fontSize: 32, marginBottom: 8, opacity: 0.3 }}>📭</div>
                <p style={{ fontSize: 12 }}>{t('noMyTickets')}</p>
              </div>
            ) : (
              myTickets.map(tk => (
                <div key={tk.id} style={{ padding: '14px 18px', borderBottom: '1px solid var(--border)' }}>
                  ...
                  <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>
                    {tk.category} · {new Date(tk.createdAt).toLocaleDateString()}
                  </div>
                  {tk.reply && (
                    <div style={{ marginTop: 8, padding: '8px 12px', background: 'rgba(52,211,153,0.08)', border: '1px solid rgba(52,211,153,0.2)', borderRadius: 8 }}>
                      <div style={{ fontSize: 10, fontWeight: 700, color: 'var(--accent)', marginBottom: 4 }}>{t('adminReply')}</div>
                      <div style={{ fontSize: 12, color: 'var(--text-secondary)' }}>{tk.reply}</div>
                    </div>
                  )}
                </div>
              ))
