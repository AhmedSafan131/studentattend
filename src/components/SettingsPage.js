...
const SettingsPage = () => {
  ...
  const [university, setUniversity] = useState({
    ...
    phone:        '+20 48 123 4567',
    website:      'www.mnu.edu.eg',
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }} dir={isRTL ? 'rtl' : 'ltr'}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          <div style={{ padding: 16, background: 'var(--bg-card2)', borderRadius: 10, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              ...
              <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>{t('toggleThemeDesc')}</div>
            </div>
            <button onClick={toggleTheme} style={{
              padding: '8px 18px', borderRadius: 8, border: '1px solid var(--border)',
              background: theme === 'dark' ? 'rgba(96,165,250,0.1)' : 'rgba(245,158,11,0.1)',
          ...
          <div style={{ padding: 16, background: 'var(--bg-card2)', borderRadius: 10, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              ...
              <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>{t('toggleLangDesc')}</div>
            </div>
      ...
      <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
          {[
            { label: t('uniNameEn'),    key: 'name'         },
            { label: t('uniNameAr'),    key: 'nameAr'       },
            { label: t('adminEmail'),   key: 'email'        },
            ...
            <div key={f.key}>
              ...
              <input value={university[f.key]} onChange={e => setU(f.key, e.target.value)} style={inp} />
            </div>
          ))}
          <div>
            <label style={lbl}>{t('totalWeeks')}</label>
            <input type="number" min={1} max={20} value={university.totalWeeks}
              onChange={e => setU('totalWeeks', +e.target.value)} style={inp} />
          </div>
        </div>
        ...
        {notifications.lowAttendanceAlert && (
          <div>
            <label style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-muted)', display: 'block', marginBottom: 6 }}>
              {t('lowThreshold')} — {notifications.lowThreshold}%
            </label>
            <input type="range" min={30} max={90} step={5} value={notifications.lowThreshold}
              onChange={e => setN('lowThreshold', +e.target.value)}
      ...
      <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
        ...
        <div>
          <label style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-muted)', display: 'block', marginBottom: 6 }}>
            {t('sessionTimeout')} — {security.sessionTimeout} {t('minutes')}
          </label>
          <input type="range" min={15} max={240} step={15} value={security.sessionTimeout}
            onChange={e => setS('sessionTimeout', +e.target.value)}
            style={{ width: '100%', accentColor: 'var(--primary)' }}
          />
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10, color: 'var(--text-muted)', marginTop: 2 }}>
            <span>15</span><span>240</span>
          </div>
        </div>
      ...
      <div className="card" style={{ border: '1px solid rgba(239,68,68,0.2)', display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        {[
          { label: t('clearData'),   desc: t('clearDataDesc'),   btn: t('clearDataBtn')   },
          { label: t('resetDoctors'), desc: t('resetDoctorsDesc'), btn: t('resetDoctorsBtn') },
        ].map(item => (
          <div key={item.label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 16px', background: 'rgba(239,68,68,0.05)', borderRadius: 10 }}>
            ...
            <button style={{ padding: '8px 18px', borderRadius: 8, border: '1px solid rgba(239,68,68,0.35)', background: 'rgba(239,68,68,0.1)', color: '#f87171', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
              {item.btn}
            </button>
          </div>
        ))}
