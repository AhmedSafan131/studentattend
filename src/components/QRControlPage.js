    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }} dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <div className="card">
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 18 }}>
          ...
          <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-primary)' }}>
            {t('liveSessions')} ({activeSessions.length} {t('active')})
          </span>
        </div>
        {activeSessions.length === 0 ? (
          <div style={{ textAlign: 'center', color: 'var(--text-muted)', padding: 30 }}>{t('noActiveSessions')}</div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {activeSessions.map((s, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '12px 16px', background: 'var(--bg-card2)', borderRadius: 10, border: '1px solid rgba(52,211,153,0.15)' }}>
                <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'linear-gradient(135deg,#1a6b45,#34d399)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 700, fontSize: 13, flexShrink: 0 }}>
                  {s.doctor.split(' ').map(w => w[0]).slice(1, 3).join('')}
                </div>
                ...
                <div style={{ textAlign: 'center' }}>
                  <div style={{ fontSize: 20, fontWeight: 800, color: '#34d399' }}>{s.scanned}</div>
                  <div style={{ fontSize: 10, color: 'var(--text-muted)' }}>{t('scanned')}</div>
                </div>
                <div style={{ textAlign: 'center' }}>
                  <div style={{ fontSize: 12, color: 'var(--text-secondary)' }}>{s.since}</div>
                  <div style={{ fontSize: 10, color: 'var(--text-muted)' }}>{t('started')}</div>
                </div>
                <button style={{ padding: '5px 12px', borderRadius: 8, border: '1px solid rgba(239,68,68,0.3)', background: 'rgba(239,68,68,0.08)', color: '#f87171', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
                  {t('forceEnd')}
                </button>
              </div>
            ))}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
        ...
        <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div>
            <label style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-muted)', display: 'block', marginBottom: 6 }}>
              {t('qrExpiry')} — {settings.expirySeconds}s
            </label>
            <input type="range" min={10} max={120} step={5} value={settings.expirySeconds}
              onChange={e => set('expirySeconds', +e.target.value)}
          ...
          <div>
            <label style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-muted)', display: 'block', marginBottom: 6 }}>
              {t('qrSize')} — {settings.codeSize}px
            </label>
            <input type="range" min={180} max={350} step={10} value={settings.codeSize}
              onChange={e => set('codeSize', +e.target.value)}
              style={{ width: '100%', accentColor: 'var(--accent)' }}
            />
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10, color: 'var(--text-muted)', marginTop: 2 }}>
        ...
        <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          {settings.allowLate && (
            <div>
              <label style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-muted)', display: 'block', marginBottom: 6 }}>
                {t('lateWindow')} — {settings.lateWindowMins} {t('minutes')}
              </label>
              <input type="range" min={0} max={30} step={5} value={settings.lateWindowMins}
                onChange={e => set('lateWindowMins', +e.target.value)}
                style={{ width: '100%', accentColor: '#f59e0b' }}
              />
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10, color: 'var(--text-muted)', marginTop: 2 }}>
                <span>0</span><span>30 {t('minutes')}</span>
              </div>
            </div>
          )}
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
        <button onClick={handleSave} style={{
          ...
          background: saved ? 'linear-gradient(135deg,#059669,#34d399)' : 'linear-gradient(135deg,#1a6b45,#22874f)',
          color: '#fff', fontWeight: 700, fontSize: 14, cursor: 'pointer',
          boxShadow: '0 4px 14px rgba(26,107,69,0.3)', transition: 'all 0.3s',
        }}>
...
const Toggle = ({ label, desc, value, onChange }) => (
  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
    ...
    <button onClick={() => onChange(!value)} style={{
      ...
      background: value ? 'var(--accent)' : 'rgba(255,255,255,0.12)', position: 'relative', transition: 'background 0.25s',
    }}>
